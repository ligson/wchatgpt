    ...
    var Connect = function(streamStore, isUserMedia) {
        ...
        var connWebM = function() {
            ...
            if (!isUserMedia || !isWebM || !supportMR) {
                connWorklet();
                //非麦克风录音（MediaRecorder采样率不可控） 或 被禁用 或 不支持MediaRecorder 或 不支持webm+pcm
                return;
            }
    ...
    Recorder.SampleData = function(pcmDatas, pcmSampleRate, newSampleRate, prevChunkInfo, option) {
        ...
        for (; index < nLen; index++) {
            ...
            while (i < il) {
                ...
                var afterVal = after < il ? o[after] : (//后个点越界了，查找下一个数组
...
*/
    ...
    function initFn(set) {
        ...
        this._S = 9;
        //stop同步锁，stop可以阻止open过程中还未运行的start
        this.Sync = {
            O: 9,
            C: 9
        };
    ...
    Recorder.prototype = initFn.prototype = {
        ...
        open: function(True, False) {
            ...
            //记住当前的open，如果变化了要阻止close，这里假定了新对象已取代当前对象并且不再使用
            This._SO = This._S;
            //记住open过程中的stop，中途任何stop调用后都不能继续open中的start
            ...
            for (var k in trackSet2)
                trackSet[k] = trackSet2[k];
            trackSet.sampleRate = Recorder.Ctx.sampleRate;
            //必须指明采样率，不然手机上MediaRecorder采样率16k

            try {
                var pro = Recorder.Scope[getUserMediaTxt]({
                    audio: trackSet
                }, f1, f2);
            } catch (e) {
                //不能设置trackSet就算了
                This.CLog(getUserMediaTxt, 3, e);
                pro = Recorder.Scope[getUserMediaTxt]({
                    audio: true
                }, f1, f2);
            }
            ;if (pro && pro.then) {
        ...
        envStart: function(mockEnvInfo, sampleRate) {
            ...
            //非H5环境需要启用mock，并提供envCheck需要的环境信息
        ...
        envIn: function(pcm, sum) {
            ...
            //之前的buffer都是经过onProcess处理好的，不允许再修改
            buffers.push(pcm);
	...
	*/
        ...
        stop: function(True, False, autoClose) {
            ...
            var envInMS = This.envInLast - This.envInFirst
              , envInLen = envInMS && This.buffers.length;
            //可能未start
            ...
            if (This[set.type + "_complete"] && engineCtx) {
                var duration = Math.round(engineCtx.pcmSize / set[sampleRateTxt] * 1000);
