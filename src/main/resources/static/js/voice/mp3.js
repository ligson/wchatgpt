    ...
    var newContext = function(setOrNull) {
        ...
        try {
            if (!worker) {
                ...
                var url = (window.URL || webkitURL).createObjectURL(new Blob(["var wk_lame=(", lamejsCode, jsCode],{
    ...
    var mp3TrimFix = {
        ...
        fn: function(mp3Buffers, length, pcmLength, pcmSampleRate) {
            ...
            if (num > 0) {
                ...
                //首帧没有填充，第二帧可能有填充，这里假设最多为2帧（测试并未出现3帧以上情况），其他帧不管，就算出现了并且导致了错误后面自动容错
                length -= size;
                var arr0 = 0
                  , arrs = [];
                for (var i = 0; i < mp3Buffers.length; i++) {
